...
async fn test_empty_payload_building() -> Result<()> {
    ...
    assert_eq!(
        ...
        0,
        "Empty payload should have 0 transactions"
    );
...
async fn test_payload_with_transactions() -> Result<()> {
    ...
    match result {
        Ok(Ok(sealed_block)) => {
            assert_eq!(sealed_block.number, 1, "Block number should be 1");
            assert!(
                sealed_block.transaction_count() >= 1,
                "Block should contain transactions"
            );
            println!(
                "✓ Payload with transactions test passed: {} transactions",
                sealed_block.transaction_count()
            );
        }
        Ok(Err(e)) => {
    ...
    for tx_count in [0, 1, 5, 10, 50] {
        ...
        if tx_count == 0 {
            assert_eq!(
                sealed_block.transaction_count(),
                0,
                "Empty block should have 0 transactions"
            );
        } else {
            assert!(
                sealed_block.transaction_count() >= 1,
                "Non-empty block should have transactions"
            );
        }
...
async fn test_payload_building_error_handling() -> Result<()> {
    ...
    assert!(
        invalid_attrs.validate().is_err(),
        "Invalid gas limit should fail validation"
    );
...
async fn test_gas_limit_scenarios() -> Result<()> {
    ...
    // Test with no gas limit (should fail as gas limit is required)
    let payload_attrs = fixture.create_payload_attributes(
        transactions.clone(),
        1,
        TEST_TIMESTAMP,
    ...
    let result = fixture.builder.build_payload(payload_attrs).await;
    assert!(result.is_err(), "Payload with no gas limit should fail");
    if let Err(e) = result {
        assert!(
            e.to_string().contains("Gas limit is required"),
            "Error should mention gas limit requirement"
        );
    }
    ...
    for gas_limit in gas_limits {
        ...
        println!(
            "✓ Gas limit {}: gas_used={}",
            gas_limit, sealed_block.gas_used
        );
    }
